...
exports.getSpecialPrices = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error al obtener precios especiales', error: error.message });
  }
};
...
exports.createSpecialPrice = async (req, res) => {
  try {
    ...
    res.status(error.message === 'El precio especial ya existe' ? 409 : 500).json({ message: error.message });
  }
};
...
exports.updateSpecialPrice = async (req, res) => {
  ...
  try {
    const specialPrice = await specialPriceService.updateSpecialPrice(id, product, price);
    res.status(200).json({ product: specialPrice.product, price: specialPrice.price });
  } catch (error) {
